...
function formatData($type,$tag,$intel,$data) {
	...
	if ($type == 'ASCII') {
		...
		if ($tag == '010f') { // Format certain kinds of strings nicely (Camera make etc.)
			$data = ucwords(strtolower(trim($data)));
		} else if ($tag == '0132' || $tag == '9003' || $tag == '9004') { // DateTime
...
function read_exif_data_raw($path,$verbose) {
	...
	if (isset($source)) {
		...
		if (preg_match_all('/[aA]ux:(?<tag>[^>="]+)(>|=")(?<data>[^<"]+)/', $xmpdata, $aux)) {
			$result['ValidAUXData'] = 1;
			$result['AUX'] = array_combine($aux['tag'], $aux['data']);
		}
		unset($source);
	}
